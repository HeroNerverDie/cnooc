...
var mocImplementationRequestBasicInfo =
  this.$store.getters.nebulogyForm.nebulogyForm
    .mocImplementationRequestBasicInfo;
mocImplementationRequestBasicInfo.folio =
  this.$store.getters.nebulogyForm.nebulogyForm.formInfo.folio;
if (
  this.getView() == 'launch' &&
  (this.getUrlData('draftId') == null || this.getUrlData('draftId') === '')
) {
...
if (this.getView() !== 'launch') {
  if (
    this.$store.getters.nebulogyForm.nebulogyForm.mocReadinessReviewChecklist
      .length == 0
  ) {
      ...
      .then((json) => {
        if (json.data.length > 0) {
          json.data.forEach((item) => {
            var info = {};
            info.content =
              item.assessmentContent_en + item.assessmentContent_zh;
  ...
  if (
    this.$store.getters.nebulogyForm.nebulogyForm.mocFieldVerificationChecklist
      .length == 0
  ) {
...
if (this.getView() != 'launch') {
  ...
  let htm =
    this.$store.getters.nebulogyForm.nebulogyForm
      .mocImplementationRequestBasicInfo.mocNumber;
  ...
  this.$request({
    ...
    .then((response) => {
      if (
        response.currentActivitys != null &&
        response.currentActivitys.length > 0 &&
