...
// 点击批量下载事件
async function downloadFilesWithToken(filelist) {
  var token = JSON.parse(localStorage.getItem('PAI-token')).access_token; // 从Local Storage获取token
  if (!token) {
    console.error('Token not found');
    return;
  }

  //var fileUrls = filelist.map(file => window.BASE_API + "file-rest/api/file/authDownload" + file.fileUrl);
  var failInfo = [];
  filelist.forEach((item) => {
    failInfo.push({
      url: window.BASE_API + 'file-rest/api/file/authDownload' + item.fileUrl,
  ...
  try {
    await Promise.all(
      failInfo.map((item) =>
        downloadFile(item.url, item.fileName, item.fileType, token)
      )
    );
...
async function downloadFile(url, fileName, fileType, token) {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Network response was not ok for URL: ${url}`);
        ...
        link.download = fileName + '.' + fileType;  // 指定下载文件名;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
  } catch (error) {
    console.error(
      `There has been a problem with your fetch operation for URL: ${url}`,
      error
    );
  }
}
...
// Example usage:
var filelist = this.$store.getters.nebulogyForm.nebulogyForm.fileInfo;
downloadFilesWithToken(filelist);
...
if (fileInfo != null && fileInfo.length > 0) {
    ...
    if (this.getView() != 'launch') {
        this.$request({
            ...
            info.nodeName = json.currentActivitys[0].activityName;
