...
if(action.actionName == "Accept" || action.actionName == 'reSubmit'){
   ...
   let message = window.lang == 'zh-CN' ? '请选择Moc审批流程' : 'Please select Moc approval process.'
   if (!moc) {
	 this.$message({
	   message: message,
	   type: 'warning'
...
if (window.currentNode == '020_Readiness Review') {
  ...
  if(type == 'LSMOC'){
	...
	if(tmocDate.getTime() < readinessDate.getTime()){
	  let message = window.lang == 'zh-CN' ? 'TMOC到期日必须大于就绪日期' : 'TMOC expiration date must be greater than the readiness date.'
	  this.$message({
		  message: message,
		  type: 'warning'
		});
	...
	if ((tmocDate - readinessDate) / (1000 * 60 * 60 * 24) > 180) {  
		// 显示提示语并阻止进一步操作（如果有需要）  
	   let message = window.lang == 'zh-CN' ? 'TMOC到期日不可超过准备完成日期180天' : 'TMOC due date cannot be greater than 180 days of readiness done date.'
		this.$message({
		  message: message,
		  type: 'warning'
		});
...
if(action.actionName == 'Reject'){
  ...
  if(!content){
	let message = window.lang == 'zh-CN' ? '请在“评论”中输入退回原因！' : 'Please enter the reason for return in the "Comment" section!'
	this.$message({
	   message: message,
	   type: 'warning'
	 });
...
if(action.actionName == 'PA_JabilCancel'){
  let content = this.$refs.textarea1.getValue();
  if(!content){
	let message = window.lang == 'zh-CN' ? '请在“评论”中输入拒绝原因！' : 'Please enter the reject reason in the "comments"!'
	this.$message({
	   message: message,
	   type: 'warning'
	 });
