...
if (this.getView() == 'launch' && (this.getUrlData('draftId') == null || this.getUrlData('draftId') === '')) {
    ...
    this.$refs.applicantName.setValue(empName);
    mocImplementationRequestBasicInfo._applicantName = empName;
    //流水号
    this.$refs.folio.setValue(window.lang == 'zh_CN' ? '自动生成' : 'Auto Create');
  //申请人邮箱
    this.$store.getters.nebulogyForm.nebulogyForm.emailContent.applicationEmail = emp.email;
    //流程摘要
    if(window.lang=="en"){
        this.$store.getters.nebulogyForm.nebulogyForm.emailContent.subject='MOC Implementation Request Workflow'
    }else{
        this.$store.getters.nebulogyForm.nebulogyForm.emailContent.subject='MOC确认检查'
    }
    //邮件使用字段--申请人   
    this.$store.getters.nebulogyForm.nebulogyForm.emailContent.applicant=empName;
}

//退回到任意节点的邮件通知，发送给申请人
window.emailToapplication = function (action) {
  debugger
  if (action.actionName == "Reject" || action.actionName == "PA_ReturnOriginator" || action.actionName == "PA_JabilCancel" || action.actionName == "PA_ReturnPrevious") {
    ...
    var _templateId = "public_return_email";
    if (action.actionName == "PA_JabilCancel") {
        _templateId = "public_reject_email";
    }
    ...
    _this.$request({
        ...
        data: {
            ...
            templateId: _templateId, //退回邮件模版
            receive: email,
            formData: {
                ...nebulogyForm,
                approverDesc: _this.$refs.approverDesc.getValue()
            },
        },
        headers: null,
    }).then(res => {
...
window.setDisplayName = function () {
    ...
    if (_this.$refs.applicantName) { customKey.formInfo.requestor = _this.$refs.applicantName.getValue(); }
}
...
window.getMOCConfirmItems = function () {
    _this.$request({
        ...
        if (response.mocConfirmItemsList.length > 0 && response.responseCode === '100') {
            debugger
            //打印模版使用
            if (window.currentNode == '010_Applicant confirmation' || window.currentNode == 'Start' || window.currentNode == '重新提交') {
...
if (this.getView() == 'launch') {
    ...
    this.$refs.applicantName.setValue(employeeName);
...
window.showComp = function () {
    ...
    if (mocType == 'TMOC' || mocType == 'LSMOC') {
        ...
        // 010节点：可编辑
        if (window.currentNode == "010_Applicant confirmation" || _this.getView() == 'resubmit') {
    ...
    if (mocType === 'PMOC' || mocType === 'TMOC') {
        ...
        //     // 现场验证检查表 查询表数据
        //     _this.$refs.sectioncols16.show();
        //     _this.$refs.datagrid1.setValue(_this.$store.getters.nebulogyForm.nebulogyForm.mocFieldVerificationChecklist);
        //     // 准备就绪审查清单
        //     _this.$refs.sectioncols17.hide();
        // }
    }
...
window.getCurrentProcInst = function () {
    _this.$request({
        ...
        if (response.currentActivitys != null && response.currentActivitys.length > 0 && response.responseCode === '100') {
            ...
            let nebulogy = _this.$store.getters.nebulogyForm.nebulogyForm;
            if(nebulogy.emailContent){
              _this.$store.getters.nebulogyForm.nebulogyForm.emailContent.currentActivityName = response.currentActivitys[0].displayName;
            }else{
                _this.$store.getters.nebulogyForm.nebulogyForm.emailContent = {
                    currentActivityName: response.currentActivitys[0].displayName,
                    applicant: emp.employeeName ? emp.employeeName : "",
                    applicationEmail: emp.email,
                    subject: window.lang=="en" ? 'MOC Implementation Request Workflow' : 'MOC确认检查'
                }
            }
            ...
            if (window.currentNode !== '重新提交') {
                _this.$refs.button1.hide();
            }
            // 刷新上传附件表格
            let upData = JSON.parse(JSON.stringify(_this.$refs.dgFileList.getValue()));
            _this.$refs.dgFileList.setValue(upData)
            // 根据需求隐藏组件
        }
        window.getMOCConfirmItems()
        window.showComp();
...
// 获取当前节点信息
if (this.getView() != 'launch' && window.currentNode != 'start') {
    // 给变更编号赋值
    ...
    _this.$store.getters.nebulogyForm.nebulogyForm.emailContent.currentActivityName = { en: "ProcessStart", zh_CN: "流程开始" };
}
...
if (this.getView() == 'launch' || this.getView() == 'resubmit') {
    ...
    let mocType = this.$store.getters.nebulogyForm.nebulogyForm.mocImplementationRequestBasicInfo.mocType;
    if (!mocType) {
        // 发起视图下 初始化隐藏现场验证检查表 准备就绪审查清单(草稿显示现场检查表)
