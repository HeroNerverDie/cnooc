...
if (window.currentNode == '010_Applicant confirmation' || window.currentNode == 'Start') {
	if (window.mocType == 'LSMOC') {
		let initiator = this.$refs.applicantName.getValue();
		let operatingNominee = this.$refs.choosebox2.displayValue;
		if (initiator == operatingNominee) {
			if (lang == 'zh_CN') {
				this.$message({
					message: '流程发起人和操作代表不能为同一个人',
					type: 'warning'
				});
				 return false;
			} else {
				this.$message({
					message: 'The initiator of the process and the representative of the operation cannot be the same person.',
					type: 'warning'
				});
				 return false;
			}
...
let mocInfo = this.$store.getters.nebulogyForm.nebulogyForm.mocImplementationRequestBasicInfo
if (mocInfo.cc != "") {
  this.$store.getters.nebulogyForm.nebulogyForm.ccApprover = this.$refs.choosebox3.displayValue;
  this.$store.getters.nebulogyForm.nebulogyForm.emailContent.ccApprover = this.$refs.choosebox3.displayValue;
}
